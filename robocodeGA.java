...
public class robocodeGA extends FitnessFunction{
	...
	public static final int CHROMOSOME_AMOUNT = 7;
	// track scores
	...
	public void run() throws Exception {
		...
		sampleGenes[0] = new DoubleGene(conf, 1, 15 ); // Hits to switch movement
		sampleGenes[1] = new DoubleGene(conf, 1000, 10000); // Interval between hits
		sampleGenes[2] = new DoubleGene(conf, 3000, 9000);
		sampleGenes[3] = new DoubleGene(conf, 2, 10); // Max velocity in circle movement strategy
		sampleGenes[4] = new DoubleGene(conf, 2, 10); // Max velocity in zig zag movement strategy
		sampleGenes[5] = new DoubleGene(conf, 0.5, 3.0);
		sampleGenes[6] = new DoubleGene(conf, 1, 100);
	...
	public boolean battleResults(String name,int score){
		String same = "albertoErick.AlbertoErickRobotGA*"; // enter robot name here with folder prefix
    ...
    protected double evaluate( IChromosome chromosome) {
        ...
        RobocodeEngine engine = new RobocodeEngine(new java.io.File("C:\robocode")); // create robocode engine
        ...
        RobotSpecification[] selectedRobots = engine.getLocalRepository("sample.Corners,albertoErick.AlbertoErickRobotGA"); // which sample bots to take to battle
